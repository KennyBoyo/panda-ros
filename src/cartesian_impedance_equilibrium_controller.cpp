...
bool CartesianImpedanceEquilibriumController::init(hardware_interface::RobotHW* robot_hw,
  ...
  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 20, &CartesianImpedanceEquilibriumController::equilibriumPoseCallback, this,
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceEquilibriumController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceEquilibriumController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceEquilibriumController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceEquilibriumController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceEquilibriumController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceEquilibriumController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceEquilibriumController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  //     boost::bind(&CartesianImpedanceEquilibriumController::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceEquilibriumController::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceEquilibriumController::update(const ros::Time& /*time*/,
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceEquilibriumController::saturateTorqueRate(
...
void CartesianImpedanceEquilibriumController::complianceParamCallback(
